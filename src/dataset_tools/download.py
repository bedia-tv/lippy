'''This module handles downloading the audio,
...
def get_options():
    ...
    subs = YoutubeDL(ydl_subs_opts)
    audio = YoutubeDL(ydl_audio_opts)
    video = YoutubeDL(ydl_video_opts)

    return [subs, audio, video]


def get_transcript(videoid):
    '''Given a video file saves and returns the context of the
    given ttml file.'''
...
def save_transcript(cleantext, videoid):
    '''Given a transcript file, converts it to a .txt file and
    removes the original .ttml file.'''
