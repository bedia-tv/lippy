...
class GetFaceBoundsTest(TestCase):
    ...
    def test_bound_same_faces(self):
        ...
        self.assertEqual(
            EXPECTED_RESULT, get_face_bounds(self.MIDDLE_FACE, self.MIDDLE_FACE)
        )

    def test_bound_different_faces(self):
        EXPECTED_RESULT = {"left": 0, "right": 15, "top": 0, "bottom": 15}
        self.assertEqual(
            EXPECTED_RESULT, get_face_bounds(self.TOP_LEFT_FACE, self.BOTTOM_RIGHT_FACE)
        )
...
class CropWordTest(TestCase):
    ...
    def test_no_crop_on_multiple_start_faces(self, mock_get_faces, mock_apply_padding):
        mock_get_faces.side_effect = [[self.FACE_ONE, self.FACE_TWO], [self.FACE_ONE]]
        mock_apply_padding.return_value = (self.START, self.END)
        capturedOutput = StringIO()
        sys.stdout = capturedOutput
        with patch("src.build_dataset.video_crop.VideoFileClip") as mock_video_file_clip:
    ...
    def test_no_crop_on_multiple_end_faces(self, mock_get_faces, mock_apply_padding):
        mock_get_faces.side_effect = [[self.FACE_ONE], [self.FACE_ONE, self.FACE_TWO]]
        mock_apply_padding.return_value = (self.START, self.END)
        capturedOutput = StringIO()
        sys.stdout = capturedOutput
        with patch("src.build_dataset.video_crop.VideoFileClip") as mock_video_file_clip:
